...
{
    // Register custom receivers for QML access
    qmlRegisterUncreatableType<CustomPointCloudReceiver>(
        "Custom.PointCloud", 1, 0, "CustomPointCloudReceiver",
        "Access via CustomPlugin.pointCloudReceiver"
    );
    ...
    qmlRegisterType<PointCloudGeometry>(
        "Custom.PointCloud", 1, 0, "PointCloudGeometry"
    );
    qmlRegisterType<PointCloudGridGeometry>(
        "Custom.PointCloud", 1, 0, "PointCloudGridGeometry"
    );

    // Start custom TCP receivers from [Network] settings.
    if (_pointCloudReceiver) {
        _pointCloudReceiver->setAutoReconnect(true);
        _pointCloudReceiver->connectToConfiguredServer();
    }
    if (_statusReceiver) {
        _statusReceiver->setAutoReconnect(true);
        _statusReceiver->connectToConfiguredServer();
    }
}
...
{
    // Innovatech logo for dark backgrounds
    return QStringLiteral("/custom/img/InnovatechLogo.svg");
}

QString CustomPlugin::brandImageOutdoor(void) const
{
    // Innovatech logo for light backgrounds
    return QStringLiteral("/custom/img/InnovatechLogo.svg");
}

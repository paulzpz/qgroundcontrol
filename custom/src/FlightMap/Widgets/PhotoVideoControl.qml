 ...
 * Custom modifications for Innovatech Control
 *
...
Rectangle {
    ...
    // Innovatech: Storage estimation settings (in MB)
    property real   _photoSizeMB:               8.0     // Average photo size in MB (adjustable)
    property real   _videoBitrateMBperMin:      150.0   // Video bitrate in MB per minute (adjustable for 4K ~100-200MB/min)

    // Innovatech: Calculated storage estimates
    property int    _estimatedPhotos:           _camera.storageFree > 0 ? Math.floor(_camera.storageFree / _photoSizeMB) : 0
    property int    _estimatedVideoMinutes:     _camera.storageFree > 0 ? Math.floor(_camera.storageFree / _videoBitrateMBperMin) : 0
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    // Innovatech: Enhanced storage display with photo/video estimates
                    Rectangle {
                        Layout.alignment:   Qt.AlignHCenter
                        Layout.fillWidth:   true
                        height:             storageEstimateColumn.height + _smallMargins * 2
                        color:              Qt.rgba(qgcPal.windowShade.r, qgcPal.windowShade.g, qgcPal.windowShade.b, 0.7)
                        radius:             _smallMargins
                        visible:            _camera.storageStatus === MavlinkCameraControl.STORAGE_READY && _camera.storageFree > 0

                        ColumnLayout {
                            id:                 storageEstimateColumn
                            anchors.centerIn:   parent
                            spacing:            2
                            ...
                            RowLayout {
                                ...
                                QGCLabel {
                                    text:               "~" + _estimatedPhotos.toLocaleString()
                                    font.pointSize:     ScreenTools.smallFontPointSize
                                    font.bold:          true
                                }
                            ...
                            RowLayout {
                                ...
                                QGCLabel {
                                    text:               "~" + _estimatedVideoMinutes + " " + qsTr("min")
                                    font.pointSize:     ScreenTools.smallFontPointSize
                                    font.bold:          true
                                }
